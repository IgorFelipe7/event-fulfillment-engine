    ...
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-lg relative z-10">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-400 font-medium tracking-wide">Cadastro Oficial</p>
        </div>
          ...
          <div className="bg-[#0a0a0a]/80 backdrop-blur-2xl border border-white/10 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <input type="text" required placeholder="Digite seu nome" value={formData.nome} onChange={e => setFormData({ ...formData, nome: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-[#b1bbe8] transition-all placeholder:text-gray-600" />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-[0.2em] font-bold text-gray-500 ml-2">WhatsApp</label>
                <input type="tel" required placeholder="(19) 99999-9999" value={formData.whatsapp} onChange={handlePhoneChange} maxLength={15} className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-[#b1bbe8] transition-all placeholder:text-gray-600" />
              </div>

              <div className="space-y-2 relative" ref={dropdownRef}>
                <label className="text-[10px] uppercase tracking-[0.2em] font-bold text-gray-500 ml-2">Sua Congregação</label>
                <div onClick={() => setIsDropdownOpen(!isDropdownOpen)} className={`w-full bg-white/5 border ${isDropdownOpen ? 'border-[#b1bbe8]' : 'border-white/10'} rounded-2xl px-5 py-4 text-white flex justify-between items-center cursor-pointer transition-all`}>
                ...
                {isDropdownOpen && (
                  <div className="absolute top-full left-0 w-full mt-2 bg-[#111] border border-white/10 rounded-2xl shadow-2xl max-h-60 overflow-y-auto z-50 animate-in fade-in zoom-in-95 duration-200">
                    ...
                    {CONGREGACOES.map((cong) => (
                      <div key={cong} className={`px-5 py-4 hover:bg-white/5 cursor-pointer transition-colors text-sm ${formData.congregacao === cong ? 'bg-[#3c5491]/20 text-[#b1bbe8] font-black' : 'text-gray-300 font-medium'}`} onClick={() => { setFormData({ ...formData, congregacao: cong }); setIsDropdownOpen(false); }}>{cong}</div>
                    ))}
                  </div>
                )}
              </div>

              <button type="submit" disabled={!isFormComplete || status === 'loading'} className="w-full bg-white text-[#050505] py-5 rounded-2xl font-black text-lg hover:bg-[#b1bbe8] transition-all flex items-center justify-center gap-3 disabled:opacity-30 disabled:cursor-not-allowed mt-4 shadow-[0_0_30px_rgba(255,255,255,0.1)]">
